...
import { generateCommandSkills } from "./generate-command-skills.ts";
import { diffNames, inspectUpstream, snapshotDigest } from "./upstream-files.ts";
...
try {
  ...
  run(process.execPath, ["--experimental-strip-types", join(root, "scripts", "check-upstream.ts")]);
